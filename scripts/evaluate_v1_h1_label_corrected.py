...
DATASET_PATH = Path(
    "data/processed/"
    "v0_dataset_v3_label_corrected.parquet"
)
...
V0_OOF_PATH = Path(
    "data/processed/"
    "v0_lc_xgb_a5_oof_predictions.parquet"
)

V1_OOF_PATH = Path(
    "data/processed/"
    "v1_h1_lc_oof_predictions.parquet"
)

METRICS_PATH = Path(
    "artifacts/"
    "v1_h1_lc_metrics.csv"
)

COMPARISON_PATH = Path(
    "artifacts/"
    "v1_h1_lc_vs_v0_lc_comparison.csv"
)
...
# A1 contains:
#   horizon_sec + 12 offensive-geometry features.
#
# H1 removes the explicit horizon from both heads.
...
assert len(OFFENSIVE_GEOMETRY) == 12

assert len(HEAD1_FEATURES) == 36
assert len(set(HEAD1_FEATURES)) == 36

assert len(HEAD2_FEATURES) == 27
assert len(set(HEAD2_FEATURES)) == 27
...
print(
    "V1-H1 — LABEL-CORRECTED HIERARCHICAL EVALUATION"
)
...
print(
    f"Head 1 features: "
    f"{len(HEAD1_FEATURES)}"
)

print(
    f"Head 2 features: "
    f"{len(HEAD2_FEATURES)}"
)
...
# Frozen label-corrected V0 baseline assertion.
assert abs(
    v0_result[
        "log_loss"
    ]
